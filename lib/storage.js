...
export function createStorage({ db, dir }) {
  ...
  for (const col of ["lat REAL", "lng REAL"]) {
    try {
      db.exec(`ALTER TABLE claude_entries ADD COLUMN ${col}`);
    } catch (e) {
  ...
  const insertStmt = db.prepare(`
    INSERT INTO claude_entries
      (id, created_at, image_filename, image_size, response_text, response_size, prompt, model, lat, lng)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const listStmt = db.prepare(`
    SELECT id, created_at, image_filename, image_size, response_text, response_size, prompt, model, lat, lng
    FROM claude_entries
  ...
  const listBeforeStmt = db.prepare(`
    ...
    LIMIT ?
  `);
  const listGeotaggedStmt = db.prepare(`
    SELECT id, created_at, image_filename, response_text, lat, lng
    FROM claude_entries
  ...
  function listEntries({ limit = 200, offset = 0, before } = {}) {
    if (typeof before === "number" && Number.isFinite(before)) {
      return listBeforeStmt.all(before, limit);
    }
    return listStmt.all(limit, offset);
  }

  function listGeotagged() {
    return listGeotaggedStmt.all();
  }
